...
docker run -d --name test_torrent_${DATE_TEST} xataz/rtorrent-flood

f_log INF "Wait 30 sec ..."
sleep 30
...
f_log INF "Test flood ..."
docker exec -i test_torrent_${DATE_TEST} curl http://localhost:3000 | grep "404 Not Found"
if [ $? -ne 0 ]; then
    f_log SUC "flood is ok" 
else
    f_log ERR "flood is ko"
    exit 1
fi
